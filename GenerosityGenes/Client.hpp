...
public:
	...
	Socket::Status registerOnServer(IpAddress serverIp, unsigned short serverRegPort, std::string clientName);

	/*Receives and records connected clients names to passed vector*/
	Socket::Status receiveConnectedClientsNames(std::vector<std::string>& namesVec);
...
private:
	...
	Socket::Status sendClientRecipientData(std::string clientName);
